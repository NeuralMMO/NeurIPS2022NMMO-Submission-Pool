...
class FeatureParser:
    ...
    spec = {
        ...
        "items":
        spaces.Box(low=0, high=1, shape=(12, 16), dtype=np.float32),
        "market":
        spaces.Box(low=0, high=1, shape=(170, 16), dtype=np.float32),
        "va_move":
        spaces.Box(low=0, high=1, shape=(5, ), dtype=np.float32),
        "va_attack_target":
        spaces.Box(low=0, high=1, shape=(101, ), dtype=np.float32),
    }
    ...
    def parse(
        ...
        ret = {}
        step = np.array([step/1024], dtype=np.float32)
        for agent_id in observations:
            terrain, death_fog_damage, population, reachable, va_move = self.parse_local_map(
                observations[agent_id], step)
            entity, va_target = self.parse_entity(observations[agent_id])
            items = self.parse_items(observations[agent_id])
            market = self.parse_market(observations[agent_id])
            self_entity = entity[:1, :]
            other_entity = entity[1:, :]
            ret[agent_id] = {
                "step": step,
    ...
    def parse_items(self, observation: Dict[str, ndarray]) -> ndarray:
        items_continuous = observation["Item"]["Continuous"]
        INVENTORY_CAPACITY = CompetitionConfig.ITEM_INVENTORY_CAPACITY
        inventory = items_continuous[:INVENTORY_CAPACITY, :]
        inventory[:, 0] = 0     # ID
        inventory[:, 4] = 0     # quantity
        inventory[:, 15] = 0    # equipped
        inventory = inventory/100
        return inventory

    def parse_market(self, observation: Dict[str, ndarray]) -> ndarray:
        market_continuous = observation["Market"]["Continuous"]
        market_continuous[:, 0] = 0     # id
        market_continuous[:, 3] = 0     # capacity
        market_continuous[:, 5] = 0     # tradable
        market_continuous[:, 15] = 0    # equipped
        market_continuous = observation["Market"]["Continuous"]/100
